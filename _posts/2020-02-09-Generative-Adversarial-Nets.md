...
<ol>
 <li>通过将 <b>c</b> 加入 *G* 和 *D* 中可以得到条件生成模型$$p(x|c)$$。\(p(x|c)\)</li>
 <li>通过训练一个辅助网络，学习近似推断可以被用于预测给定 **x** 情况下的 **z**。这和wake-sleep算法训练出的推理网络类似，但是它具有一个优
势，就是在生成器网络训练完成后，这个推理网络可以针对固定的生成器网络进行训练。</li>
 <li>能够近似建模所有条件概率p(x_s|x_\$)，其中S是一个</li>
 <li>桔子</li>
</ol> 
